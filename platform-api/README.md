...
На итоговом экране завершённого аудита доступны «Скачать PDF» и «Скачать Excel».
GET /api/audits/{id}/export/pdf и GET /api/audits/{id}/export/xlsx требуют обычную cookie-сессию.
Оба используют один загрузчик snapshot в read-only REPEATABLE READ транзакции Postgres;
сверяют сохранённый итог с существующим расчётом и возвращают X-Audit-Snapshot.
Несогласованный итог или черновик возвращает 409, отсутствие аудита — 404.

PDF включает клиентскую сводку и все Red Flags, без полной распечатки 150 стандартов.
Excel содержит 150 строк, полные комментарии/evidence, сравнения и поля плана работы.
Dashboard — снимок серверных данных: правки Excel не пересчитывают его и не пишут в БД.
Ответственный и срок пока отсутствуют в модели БД; в Excel оставлены пустые поля.
История ограничена рестораном, версией методики и датой выбранного аудита; baseline — текущий
сохранённый ориентир с датой и ID источника. Полный цикл программы в БД не выделен:
при повторных циклах выбирается последняя предшествующая сессия каждого Day.

Для кириллицы Nixpacks устанавливает fonts-dejavu-core, шрифты встраиваются в PDF.
Тесты GitHub Actions используют отдельную PostgreSQL 16 и публикуют только синтетические
образцы PDF/XLSX и PNG всех страниц PDF. Продакшен-секреты тестам не нужны.
